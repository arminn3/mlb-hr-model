  ...
  return (
    <div className="max-w-5xl mx-auto px-1 py-2 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Splits vs hand + head-to-head vs today's pitcher — surfaced
            immediately on open (Harper layout). Data: splits_vs_hand + bvp_stats. */}
        {(player.splits_vs_hand || (player.bvp_stats?.career && player.bvp_stats.career.abs > 0)) && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {player.splits_vs_hand && (() => {
              const s = player.splits_vs_hand!;
              const strip0 = (v: number | null) => (v == null ? "—" : v.toFixed(3).replace(/^0/, ""));
              const rows: [string, string][] = [
                [`AB vs ${s.hand}HP`, `${s.ab}`],
                [`BA vs ${s.hand}HP`, strip0(s.ba)],
                [`HR vs ${s.hand}HP`, `${s.hr}`],
                [`% of HRs vs ${s.hand}HP`, s.pct_of_hrs != null ? `${s.pct_of_hrs}%` : "—"],
                [`K rate vs ${s.hand}HP`, s.k_pct != null ? `${s.k_pct}%` : "—"],
                [`BB rate vs ${s.hand}HP`, s.bb_pct != null ? `${s.bb_pct}%` : "—"],
                [`TB per hit vs ${s.hand}HP`, s.tb_per_hit != null ? s.tb_per_hit.toFixed(2) : "—"],
              ];
              return (
                <div className="rounded-xl p-4" style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%)", border: "1px solid rgba(255,255,255,0.10)" }}>
                  <div className="text-[11px] font-bold uppercase tracking-[0.07em] text-foreground/70 mb-3">Splits vs {s.hand}HP <span className="text-muted/50 normal-case tracking-normal">· {player.name}</span></div>
                  <div className="space-y-2">
                    {rows.map(([l, v]) => (
                      <div key={l} className="flex items-center justify-between">
                        <span className="text-muted uppercase tracking-wider text-[10px]">{l}</span>
                        <span className="font-mono font-semibold text-foreground text-[13px]">{v}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              return (
                <div className="rounded-xl p-4" style={{ background: "linear-gradient(180deg, rgba(96,165,250,0.08) 0%, rgba(255,255,255,0.02) 100%)", border: "1px solid rgba(96,165,250,0.20)" }}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] text-muted/60">career H2H</span>
                  </div>
